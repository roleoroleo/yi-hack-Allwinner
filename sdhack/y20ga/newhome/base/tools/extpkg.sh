...
cd /tmp/update
cp -rf $1 /tmp/update/home_y20gam
...
if [ "$HEADER" == "5a42" ]; then
    ...
    if [[ "$SWAP" == "0" ]]; then
        ...
        if [[ $SD_PRESENT -eq 1 ]] && [[ -f /home/yi-hack/sbin/mkswap ]]; then
            ...
            /home/yi-hack/sbin/mkswap /tmp/sd/swapfile_update
    ...
    dd if=home_y20gam of=verout bs=22 count=1
    dd if=home_y20gam of=home1 bs=22 skip=1 count=999999999999999
    rm home_y20gam
    ...
    cat verin
    rm home3
    dd if=home4 of=home_y20gam.7z bs=22 skip=1 count=9999999999999
    rm home4
    mv home_y20gam.7z home4
    zpwd=$(cat pwd)
    ver_out=$(cat verout)
    ver_in=$(cat verin)
