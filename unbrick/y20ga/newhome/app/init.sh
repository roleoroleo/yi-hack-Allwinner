...
if [ -f /home/home_y20gam ]; then
    echo "---/home/home_y20gam exist, update begin---"
	dd if=/home/home_y20gam of=/tmp/newver bs=22 count=1
	...
	if [ $newver != $curver ]; then
		...
		/tmp/update/extpkg.sh /home/home_y20gam
		rm -rf /tmp/update
		rm -rf /home/home_y20gam
		#sync
		echo "update finish"
		reboot -f
	fi
	echo "---same version ? update fail---"
	rm -rf mv /home/home_y20gam
elif [ -f /tmp/sd/home_y20gam ]; then
	echo "---tmp/sd/home_y20gam exist, update begin---"
	dd if=/tmp/sd/home_y20gam of=/tmp/newver bs=22 count=1
	newver=$(cat /tmp/newver)
	if [ -f /home/homever ]; then
		curver=$(cat /home/homever)
	else
	...
	if [ $newver != $curver ]; then
		...
		mkdir /tmp/update
		cp -rf /home/base/tools/extpkg.sh /tmp/update/extpkg.sh
		/tmp/update/extpkg.sh /tmp/sd/home_y20gam
		rm -rf /tmp/update
		mv /tmp/sd/home_y20gam	/tmp/sd/home_y20gam.done
		#sync
		echo "update finish"
		reboot -f
	fi
	...
	echo "---update file(home_y20gam) Not exist---"
fi
