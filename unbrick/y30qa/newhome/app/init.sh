...
if [ -f /home/home_y30qam ]; then
    echo "---/home/home_y30qam exist, update begin---"
	dd if=/home/home_y30qam of=/tmp/newver bs=22 count=1
	...
	if [ $newver != $curver ]; then
		...
		/tmp/update/extpkg.sh /home/home_y30qam
		rm -rf /tmp/update
		rm -rf /home/home_y30qam
		#sync
		echo "update finish"
		reboot -f
	fi
	echo "---same version ? update fail---"
	rm -rf mv /home/home_y30qam
elif [ -f /tmp/sd/home_y30qam ]; then
	echo "---tmp/sd/home_y30qam exist, update begin---"
	dd if=/tmp/sd/home_y30qam of=/tmp/newver bs=22 count=1
	newver=$(cat /tmp/newver)
	if [ -f /home/homever ]; then
		curver=$(cat /home/homever)
	else
	...
	if [ $newver != $curver ]; then
		...
		mkdir /tmp/update
		cp -rf /home/base/tools/extpkg.sh /tmp/update/extpkg.sh
		/tmp/update/extpkg.sh /tmp/sd/home_y30qam
		rm -rf /tmp/update
		mv /tmp/sd/home_y30qam	/tmp/sd/home_y30qam.done
		#sync
		echo "update finish"
		reboot -f
	fi
	...
	echo "---update file(home_y30qam) Not exist---"
fi
