...
int main(int argc, char **argv) {
    ...
    int frame_len_stored = -1, frame_counter_stored = -1;
...
//        if (debug) fprintf(stderr, "found buf_idx_2: %08x\n", (unsigned int) buf_idx_2);
        ...
        if (write_enable) {
            tb = time(NULL);
            if (buf_idx_start + frame_len_stored > addr + BUF_SIZE) {
                fwrite(buf_idx_start, 1, addr + BUF_SIZE - buf_idx_start, stdout);
                fwrite(addr + BUF_OFFSET, 1, frame_len_stored - (addr + BUF_SIZE - buf_idx_start), stdout);
            } else {
                fwrite(buf_idx_start, 1, frame_len_stored, stdout);
            }
            ta = time(NULL);
            if (ta - tb > 3) {
        ...
        if (memcmp(buf_idx_1, sps_addr, sps_len) == 0) {
            ...
            frame_counter = (int) buf_idx_1[18] + (int) buf_idx_1[19] *256;
            buf_idx_1 = cb_move(buf_idx_1, FRAME_HEADER_SIZE);
            if (frame_counter == frame_counter_stored + 1) {
                frame_len_stored = frame_len;
                frame_counter_stored = frame_counter;
                write_enable = 1;
                if (debug) fprintf(stderr, "frame detected\n");
                if (debug) fprintf(stderr, "frame_len_stored: %d\n", frame_len_stored);
                if (debug) fprintf(stderr, "frame_counter_stored: %d\n", frame_counter_stored);
                buf_idx_start = buf_idx_1;
            } else {
