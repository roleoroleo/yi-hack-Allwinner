...
/* Low performance and low memory */
int convert2jpg_lowmemory(char *output_file, char *input_file, const int width, const int height, const int dest_width, const int dest_height)
{
    struct jpeg_compress_struct cinfo;
    struct jpeg_error_mgr jerr;
    JSAMPROW row_ptr[1];
    ...
    buf_y = (unsigned char *) malloc(dest_width * sizeof(unsigned char));
    buf_uv = (unsigned char *) malloc(dest_width * sizeof(unsigned char));

    while (cinfo.next_scanline < cinfo.image_height) {
        offset = (cinfo.next_scanline + hsl/2) * width + wsl/2;                 //offset to the correct y row
        uv = width * (height - (cinfo.next_scanline + hsl/2 + 1) / 2) + wsl/2;  //offset to the correct uv row

        if(fseek(fpi, offset, SEEK_SET) < 0)
            return -1;
        if(fread(buf_y, 1, dest_width, fpi) < 2)
            return -1;
        if(fseek(fpi, offset + uv, SEEK_SET) < 0)
            return -1;
        if(fread(buf_uv, 1, dest_width, fpi) < 2)
            return -1;
        for (i = 0, j = 0; i < cinfo.image_width; i += 2, j += 6) { //input strides by 2 bytes, output strides by 6 (2 pixels)
            tmprowbuf[j + 0] = *(buf_y + i);      // Y (unique to this pixel)
            tmprowbuf[j + 1] = *(buf_uv + i);     // U (shared between pixels)
            tmprowbuf[j + 2] = *(buf_uv + i + 1); // V (shared between pixels)
            tmprowbuf[j + 3] = *(buf_y + i + 1);  // Y (unique to this pixel)
            tmprowbuf[j + 4] = *(buf_uv + i);     // U (shared between pixels)
            tmprowbuf[j + 5] = *(buf_uv + i + 1); // V (shared between pixels)
        }
        jpeg_write_scanlines(&cinfo, row_pointer, 1);
    }
